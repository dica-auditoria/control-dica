...
function DeadlineBadge({ fecha, extendida }: { fecha: string | null; extendida: boolean }) {
  ...
  if (dias > 7) {
    return (
      <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 100, background: "rgba(45,166,95,0.12)", color: "#1B7A3E", fontFamily: "'DM Mono', monospace", flexShrink: 0 }}>
        {extendida ? "Ext · " : ""}✓ {dias}d
      </span>
    );
  }
  if (dias >= 1) {
    return (
      <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 100, background: "rgba(251,191,36,0.18)", color: "#92400E", fontFamily: "'DM Mono', monospace", flexShrink: 0 }}>
        {extendida ? "Ext · " : ""}⚠ {dias}d
      </span>
    );
  }
  if (dias === 0) {
    return (
      <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 100, background: "rgba(200,71,42,0.12)", color: "#C8472A", fontFamily: "'DM Mono', monospace", flexShrink: 0 }}>
        Vence hoy
      </span>
    );
  }
