...
export async function crearEmpleadoAction(input: CrearEmpleadoInput) {
  ...
  const { data: empleado, error } = await (supabase.from("empleados") as any)
    .insert({
      ...
      hora_entrada: input.tipo_contrato === "practicas" ? (input.hora_entrada || null) : null,
      hora_salida: input.tipo_contrato === "practicas" ? (input.hora_salida || null) : null,
      tolerancia_minutos: input.tipo_contrato === "practicas" ? 10 : null,
      estado: "pendiente",
