...
async function verificarAdmin() {
  ...
  if (["admin", "superadmin", "rrhh"].includes(p?.rol ?? "")) {
  ...
  if (p?.rol === "empleado") {
    ...
    if (q) {
      ...
      if (emp && DEPTS_PUEDEN_PUBLICAR.includes(emp.departamento)) {
        return { supabase, ok: true as const, user };
      }
    }
  }

  return { supabase, ok: false as const, user };
}
